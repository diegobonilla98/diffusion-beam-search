...
# TODO: Touch hyperparameters for slower generation but better images. Run overall metrics.
...
# Switch to float16 for faster inference
dtype = torch.float16
...
def compute_forward_nll_batch(noise_pred, variance_t, d):
    ...
    """
    noise_pred = noise_pred.float()  # Cast to float32 for accurate computation
...
for step_idx, t in enumerate(tqdm(timesteps)):
    ...
    nll_current = compute_forward_nll_batch(noise_pred, variance_t, latent_dims)
    ...
    if prev_t >= 0:
        ...
        # Batch the inputs for scheduler.step
        latents_candidates = latents.repeat_interleave(num_candidates_per_beam, dim=0)
        noise_pred_candidates = noise_pred.repeat_interleave(num_candidates_per_beam, dim=0)
        
        # Use custom scheduler step with diverse noise in batch
        step_output = scheduler.step(
            model_output=noise_pred_candidates,
            timestep=t,
            sample=latents_candidates,
            generator=generator,
            return_dict=True,
            random_noise=diverse_noise
        )
        candidate_latents = step_output.prev_sample
    else:
        # No branching at last step - use scheduler step without noise, batched
        step_output = scheduler.step(
    ...
    for m in range(lookahead_steps):
        ...
        lookahead_nll = compute_forward_nll_batch(eval_noise_pred, eval_variance_t, latent_dims)
        lookahead_scores += lookahead_nll

        # Advance to next eval_latents using custom scheduler step if not last
        if m < lookahead_steps - 1:
            eval_prev_t = scheduler.previous_timestep(eval_timestep)
            
            if eval_prev_t >= 0:
                # Assuming deterministic (no noise) for lookahead advance
                # If your custom scheduler adds noise by default, pass random_noise=torch.zeros_like(eval_latents)
                step_output = scheduler.step(
                    model_output=eval_noise_pred,
                    timestep=eval_timestep,
                    sample=eval_latents,
                    generator=generator,
                    return_dict=True,
                    # random_noise=torch.zeros_like(eval_latents)  # Uncomment if needed for deterministic
                )
                eval_latents = step_output.prev_sample
            else:
                step_output = scheduler.step(
                    model_output=eval_noise_pred,
                    timestep=eval_timestep,
